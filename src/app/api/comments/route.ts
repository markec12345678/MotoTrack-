...
export async function POST(request: NextRequest) {
  try {
    ...
    // Notify the owner of the ride/route about the comment
    const relatedId = rideId || routeId || ''
    const ownerId = rideId
      ? (await db.ride.findUnique({ where: { id: rideId }, select: { userId: true } }))?.userId
      : (await db.route.findUnique({ where: { id: routeId }, select: { userId: true } }))?.userId
    if (ownerId) {
      notifyComment(ownerId, userId, text.trim(), relatedId).catch(() => {})
    }
