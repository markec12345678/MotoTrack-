...
export async function GET(request: NextRequest) {
  try {
        ...
        if (route.routeData) {
          ...
          if (Array.isArray(rd)) {
            points = rd.map((p: number[]) => ({
              lat: p[0], lng: p[1], alt: p[2] ?? null, speed: null, timestamp: p[3] || Date.now(),
            }))
          }
        } else {
          const wpRaw = JSON.parse(route.waypoints)
          if (Array.isArray(wpRaw)) {
            points = wpRaw.map((w: { lat: number; lng: number }) => ({
              lat: w.lat, lng: w.lng, alt: null, speed: null, timestamp: Date.now(),
            }))
          }
        }
      } catch { /* ignore */ }
    ...
    if (laps.length === 0 && points.length > 0) {
      laps.push({ startIndex: 0, endIndex: points.length - 1, startTime, totalDistance: distance * 1000 })
    }
...
${lapXml}
  ...
  <Author xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:type="Application_t">
    ...
    return NextResponse.json({ error: 'Napaka pri izvozu TCX' }, { status: 500 })
  }
}
