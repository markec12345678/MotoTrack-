      ...
      <div className="mx-auto max-w-lg px-4 py-6 space-y-5">
        ...
        <Card className="rounded-2xl card-hover-lift">
          <CardContent className="p-3">
            <div className="flex items-center gap-2 mb-2"><Users className="size-3.5 text-muted-foreground/50" /><span className="text-[10px] text-muted-foreground/50 font-bold uppercase tracking-wider">Uporabnik</span></div>
            <div className="flex gap-1.5 flex-wrap">
              {allUsers.map(u => (
                <Button key={u.id} variant={user.id === u.id ? 'default' : 'outline'} size="sm" className={`text-xs gap-1.5 h-7 font-semibold rounded-lg ${user.id === u.id ? '' : 'border-border/50 hover:border-primary/30'}`} onClick={() => onSwitchUser(u.id)}>
        ...
        {/* ── Improved User Card - Premium REVER style with cover gradient ── */}
        <Card className="rounded-2xl overflow-hidden card-hover-lift">
          {/* Cover gradient with decorative pattern */}
          <div className="relative h-20 bg-gradient-to-br from-primary/25 via-primary/15 to-primary/5 dark:from-primary/30 dark:via-primary/15 dark:to-transparent overflow-hidden">
            {/* Decorative elements */}
            <div className="absolute -top-4 -right-4 size-32 rounded-full bg-primary/10 blur-2xl" />
            <div className="absolute bottom-0 left-8 size-20 rounded-full bg-primary/8 blur-xl" />
            <div className="absolute top-2 right-16 size-12 rounded-full bg-white/5 blur-md" />
          </div>
          ...
          <CardContent className="p-5 -mt-10 relative">
            <div className="flex items-end gap-4">
              ...
              <div className="relative shrink-0">
                ...
                <Avatar className="size-16 relative ring-4 ring-background shadow-lg shadow-primary/10">
                  <AvatarFallback className="text-xl bg-primary/10 text-primary font-bold">{user.name.charAt(0)}</AvatarFallback>
                </Avatar>
                {/* Online indicator dot */}
                <div className="absolute bottom-0.5 right-0.5 size-4 rounded-full bg-emerald-500 ring-2 ring-background" />
              </div>
            ...
            {user.bike && <Badge className="mt-3 bg-primary/10 text-primary border-primary/15 text-[10px] h-5 font-semibold"><Bike className="size-3 mr-1" />{user.bike}</Badge>}
            {user.bio && <p className="text-xs text-muted-foreground/70 mt-2 leading-relaxed">{user.bio}</p>}
